...
String formatPriceValue(num? value, {String fallback = 'Цена'}) {
  if (value == null) {
    return fallback;
  }

  final asDouble = value.toDouble();
  final isWholeAmount = (asDouble - asDouble.roundToDouble()).abs() < 0.001;
  if (isWholeAmount) {
    return asDouble.round().toString();
  }

  var amount = asDouble.toStringAsFixed(2);
  amount = amount.replaceAll(RegExp(r'0+$'), '');
  amount = amount.replaceAll(RegExp(r'\.$'), '');
  return amount;
}

String formatEuroPrice(num? value, {String fallback = 'Цена'}) {
  if (value == null) {
    return fallback;
  }
