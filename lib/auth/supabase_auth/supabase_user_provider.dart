...
class KGPassNewSupabaseUser extends BaseAuthUser {
  KGPassNewSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> kGPassNewSupabaseUserStream() {
  ...
  final initialUser = KGPassNewSupabaseUser(SupaFlow.client.auth.currentUser);
  currentUser = initialUser;
  return supabaseAuthStream.map<BaseAuthUser>(
    (authState) {
      final user = authState.event == AuthChangeEvent.signedOut
          ? null
          : SupaFlow.client.auth.currentUser ?? authState.session?.user;
      currentUser = KGPassNewSupabaseUser(user);
      return currentUser!;
    },
